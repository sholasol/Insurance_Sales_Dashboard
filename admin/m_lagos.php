...
$rAmount = $yr['sum'];
 ...
 //Branch Wise Budget and Policy
    ...
    //Abuja
    $covArea = $con->query("SELECT count(distinct policy_no) AS count, sum(r_amount) FROM myrecord WHERE month='$m' AND year='$yy' AND c_area LIKE '%Abuja%' ");
    $cv = $covArea->fetch_array();
    $suAmt = $cv['sum(r_amount)'];
    $suPol = $cv['count'];
    
    //Lagos
    $covArea1 = $con->query("SELECT count(distinct policy_no) AS count, sum(r_amount) FROM myrecord WHERE month='$m' AND year='$yy' AND agent_code IN (SELECT agent_code FROM agent WHERE branch=$branch )     ");
               ...
               <div class="row">
                ...
                <div class="row">
                   <div class="col-md-12 col-xs-12 col-sm-6">
                                                ...
                                                <tbody>
                                                    <tr>
                                                        <th>
                                                            <strong>
                                                                <?php 
                                                                    if($rol=='FA'){echo "<a href='mbran_detail.php?m=$m&r=$rol&b=FLAG'>Bancassurance Lagos Detail</a>";}
                                                                     else{echo "<strong>Lagos  </strong>";}
                                                                ?>
                                                                
                                                            </strong>
                                                        </th>
                                                        <th><strong><span class=""><?php echo number_format($sumBud1); ?></span></strong></th>
                                                        <th><strong><span class=""><?php echo number_format($suAmt1); ?></span></strong></th>
                                                        <th><strong><span class=""> <?php echo number_format($PA_Lagos, 2, '.', '')."%"; ?></span> </strong></th>
                                                        <th><strong><span class=""><?php echo number_format($sumPol1); ?></span></strong></th>
                                                        <th><strong><span class=""><?php echo number_format($suPol1); ?></span></strong></th>
                                                        <th><strong><span class=""> <?php echo number_format($PA_Lagos1, 2, '.', '')."%"; ?></span></strong></th>
                                                    </tr>
                    ...
                    <?php if($rol =='Agency'){include 'm_lag.php';} ?>
                                ...
                                <table class="table table-bordered mb-0">
                                              ...
                                              <?php 
                                                ...
                                                while($rr2=$p2->fetch_array()){
                                                    ...
                                                    $pp2=$con->query("SELECT sum(nb) AS nb, sum(rn) AS rn, sum(polnb) AS polnb, sum(polrn) AS polrn FROM monthly WHERE month='$m' AND product_class='$pr2' AND bID='$bid' AND (branch=$branch)");
                                                    $ro2=$pp2->fetch_array();
                                                    $nb2=$ro2['nb'];
                                                    $newBizBud +=$nb2;
                                                    $rn2=$ro2['rn'];
...
<!--                                          <th colspan="4" align="center" bgcolor="#66FF99">Ibadan</strong></th>-->
                                        ...
                                        while($cr = $cp->fetch_array()){
                                            ...
                                            $pb = $con->query("SELECT sum(nb) AS nb, sum(rn) AS rn, sum(polnb) AS polnb, sum(polrn) AS polrn, product_class FROM monthly WHERE product_class='$prodt' AND month='$m' AND year='$yy' AND (branch='Ensure Victoria Island' OR branch='Ensure Ikeja' OR branch='Ensure Broad Street')");
                                            $or = $pb->fetch_array();
                                            $nbB = $or['nb'];
                                            $rnB = $or['rn'];
                                            $nbP = $or['polnb'];
                                            ...
                                            while($ro=$cpp->fetch_array()){
                                                ...
                                                //Activization for PH
                                                $acvtAbj = $con->query("SELECT count(distinct agent_code) AS agent, count(distinct policy_no) AS count, product_class FROM `myrecord` WHERE product_class='$prodt' AND type='NB' AND month='$m' AND year='$yy' AND  r_amount >= 10000 AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch=$branch ) ");
                                            ...
                                            while($ro1=$cpp1->fetch_array()){
                                                ...
                                                //Activization for PH
                                                $acvtAbj1 = $con->query("SELECT count(distinct agent_code) AS agent, count(distinct policy_no) AS count, product_class FROM `myrecord` WHERE product_class='$prodt' AND type='RN' AND month='$m' AND year='$yy'  AND r_amount >= 10000 AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch=$branch  ) ");
...
<!--    <script src="../plugins/bower_components/Chart.js/chartjs.init.js"></script>-->
  ...
  $p=$con->query("SELECT product_class FROM myrecord GROUP BY product_class ");
            while($pr=$p->fetch_array()){
               ...
               // $q1[] = 0;
            $ach= $con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count FROM `myrecord` where 
             product_class= '$produt' AND type='NB' AND  year='$yy' AND month='$m' AND  agent_code IN (SELECT agent_code FROM agent WHERE branch=$branch  ) ");
            ...
            $xxx= $yr['sum']/1000000;
             ...
             product_class= '$produt' AND  year='$yy' AND month='$m' AND type='RN'  AND  agent_code IN (SELECT agent_code FROM agent WHERE branch=$branch  )");
