...
//$agcy=$con->query("SELECT sum(nb) AS nb, sum(rn) AS rn, sum(polnb) AS polnb, sum(polrn) AS polrn FROM proclass_bud WHERE bID='$bid' AND type='Partners' AND year='$y'");
$agcy=$con->query("SELECT * FROM type WHERE bID='$bid' AND type='Partners' AND year='$y'");
$rw=$agcy->fetch_array();
$tBud=$rw['nb'] + $rw['rn'];
$tPol=$rw['polnb'] + $rw['polrn'];

$q1[] = 0;
$ach= $con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count, c_area FROM `myrecord` where c_area = 'Partners - Autodealership' AND year='$y' 
...
$agcy_ach= $con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count FROM `myrecord` where 
   year='$y' AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch='Ensure Victoria Island' OR branch='Ensure Ikeja' OR branch='Ensure Broad Street' OR branch='Ensure Abuja' OR branch='Ensure PH') 
   AND c_area ='Partners - Autodealership'");
            ...
            <div class="container-fluid">
                <div class="row bg-title">
                    <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
                        <h4 class="page-title">Partners - Autodealership </h4>
                    </div>
                    <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
                        <ol class="breadcrumb">
                           <li class="active"><a href="partner_budget.php?r=Partners" style="color: #005983;"><i class="icon-briefcase"></i> Create Budget</a> </li>
                            <li class="active"><a href="partner.php"><i class="icon-home"></i> Back</a> </li> 
                        </ol>
                    </div>
                    <!-- /.col-lg-12 -->
                </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped">
                                    ...
                                    <tbody>
                                        ...
                                        $per=$con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count, c_area FROM `myrecord` where c_area = 'Partners - Autodealership' AND year='$y' 
                                                        AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch='Ensure Victoria Island' OR branch='Ensure Ikeja' OR branch='Ensure Broad Street' OR branch='Ensure Abuja' OR branch='Ensure PH') GROUP BY c_area ORDER BY sum DESC");
...
<?php
            ...
            $ach= $con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count FROM `myrecord` where 
             product_class= '$produt' AND year='$y' AND type='NB' AND c_area = 'Partners - Autodealership' 
                  AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch='Ensure Victoria Island' OR branch='Ensure Ikeja' OR branch='Ensure Broad Street' OR branch='Ensure Abuja' OR branch='Ensure PH')
                ");
            while($yr =$ach->fetch_array()){
            ...
            $xxx= $yr['sum']/1000000;
             ...
             product_class= '$produt'  AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch='Ensure Victoria Island' OR branch='Ensure Ikeja' OR branch='Ensure Broad Street' OR branch='Ensure Abuja' OR branch='Ensure PH')
               AND year='$y' AND type='RN' AND c_area = 'Partners - Autodealership' ");
      ...
      "graphset": [{
        ...
        "labels": [{
          ...
          "tooltip": {
            "padding": 10,
            "backgroundColor": "#237b00",
            "text": ">= <?php echo $H; ?> <= <?php echo $VH; ?> <br>Units",
            "shadow": 0
          }
        }, {
          "id": "lbl2",
