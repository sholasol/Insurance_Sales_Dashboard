...
//$agcy=$con->query("SELECT  nb,  rn, polnb, polrn FROM type WHERE bID='$bid' AND type='$b' AND year='$y'");
$agcy=$con->query("SELECT sum(nb) AS nb, sum(rn) AS rn, sum(polnb) AS polnb, sum(polrn) AS polrn FROM proclass_bud WHERE bID='$bid' AND type='$b' AND year='$y'");
$rw=$agcy->fetch_array();
$tBud=$rw['nb'] + $rw['rn'];
            ...
            <div class="container-fluid">
                 <div class="row bg-title">
                    <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
                        <h4 class="page-title">
                            <?php 
                            if($b =='FA'){echo "Bancassurance";}
                            elseif($b =='Partners'){echo "Partners -Autodealership";}
                            elseif($b =='FI'){echo "Partners - FI";}
                            elseif($b =='HNI'){echo "Partners - HNI";}
                            else{ echo $b;}
                            ?>
                        </h4>
                    </div>
                ...
                <div class="row">
                   <div class="white-box">
                      <h3 class="box-title"> 
                          <?php 
                          $xx=$con->query("SELECT count(prID) AS count FROM proclass_bud WHERE bID='$bid' AND type='$b' ");
                          $rr=$xx->fetch_array();
                          $count=$rr['count'];
                          if($count < 1){echo 'Budget ';}else{echo "<a href='edit_budget.php?r=$b' class='btn btn-info'> Edit Budget</a>";}?>
                        ...
                        <div class="col-md-2 col-sm-3 col-xs-6 pull-right">
                          <select class="form-control pull-right b-none" onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
                              ...
                              <option value="sup_budget.php?r=<?php echo $b ?>">Supervisor Budget</option>
                          </select>
                        </div>
                      </h3>
                        ...
                        <table class="table table-bordered mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><span class="btn btn-info"><?php echo number_format($ptotalNB, 2, '.', ''); ?></span></td>
                                    <td><span class="btn btn-danger"><?php echo number_format($totalRN); ?></span> </td>
                                    <td><span class="btn btn-info"><?php echo number_format($ptotalRN, 2, '.', ''); ?></span></td>
