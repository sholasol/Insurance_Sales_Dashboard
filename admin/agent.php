...
$ach= $con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count, product_class FROM `myrecord` where 
             agent_code='$code' AND year='$y' AND type='NB'  GROUP BY product_class");
...
$ach= $con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count FROM `myrecord` where 
  year='$y' AND type='NB' AND `agent_code`='$code' ");
$yr =$ach->fetch_array();
...
$ach2=$con->query("SELECT sum(r_amount) as sum, count(distinct agent_code) AS agent, count(distinct policy_no) AS count FROM myrecord where 
 year='$y' AND type='RN' AND `agent_code`='$code'");
...
$AmtRN = $yr2['sum'];
 ...
 //Coverage Areas Budget
    ...
    $covArea = $con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count FROM `myrecord` where 
     year='$y' AND `agent_code`='$code' ");
    $cv = $covArea->fetch_array();
    $suAmt = $cv['sum'];
    $suPol = $cv['count'];
    
    //Number of RSEs in Each Branch
    $ag=$con->query("SELECT count(distinct policy_no) AS count, agent_name FROM `myrecord` where  year='$y' AND `agent_code`='$code' ");
...
?>
                ...
                <div class="row bg-title">
                    ...
                    <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
                        <ol class="breadcrumb">
                            <li class="active">
                                <?php 
                                 echo "<a href='detail.php?r=$rol'><i class='icon-home'></i> Back</a> ";
                                ?>
                            </li>
                        </ol>
                    </div>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-striped" width="100%">
                                                ...
                                                <tbody>
                                                    <?php
                                                    $t=0;
                                                    $q=$con->query("SELECT * FROM `myrecord` where year='$y' AND `agent_code`='$code' ");
    ...
    $p=$con->query("SELECT product_class FROM myrecord GROUP BY product_class ");
            while($pr=$p->fetch_array()){
               ...
               // $q1[] = 0;
            $ach= $con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count FROM `myrecord` where 
             product_class= '$produt' AND type='NB' AND `agent_code`='$code' ");
            while($yr =$ach->fetch_array()){
            ...
            product_class= '$produt' AND type='RN' AND `agent_code`='$code'");
            while($yr2 =$ach2->fetch_array()){
