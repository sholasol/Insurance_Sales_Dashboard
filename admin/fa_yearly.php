...
$agcy=$con->query("SELECT * FROM type WHERE bID='$bid' AND type='Financial Advisors' AND year='$y'");
$rw=$agcy->fetch_array();
$tBud=$rw['nb'] + $rw['rn'];
            ...
            <div class="container-fluid">
                <div class="row bg-title">
                    ...
                    <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
                        <ol class="breadcrumb">
                            <li class="active"><a href="fa_budget.php"><i class="icon-home"></i> Back</a> </li>
                        </ol>
                    </div>
                    <!-- /.col-lg-12 -->
                </div>
                            ...
                            <tbody>
                                <?php
                                    ...
                                    while($rw = $qq->fetch_array()){
                                        ...
                                        $pd=$con->query("SELECT * FROM proclass_bud WHERE product_class ='$prod' AND bID='2' AND type='FA'");
                                        $pr=$pd->fetch_array();
                                        $nb=$pr['nb'];
                                        $totalNB += $nb;
                                        $rn=$pr['rn'];
                ...
                <?php 
                        ...
                        $brChk = $con->query("SELECT count(prID) AS count FROM proclass_bud WHERE product_class ='$pro' AND bID='$bid' AND type='FA' AND year='$yr'");
                        $bro=$brChk->fetch_array();
                        //$cnt=$brChk->num_rows;
                        $cnt=$bro['count'];

                        if($cnt < 1){
                            $in=$con->query("INSERT INTO proclass_bud SET bID='$bid', product_class='$pro', nb='$nb', pnb='$Pnb', rn='$rn',prn='$Prn', polnb='$nbnop', ppolnb='$Pnbnop', polrn='$rnop', ppolrn='$Prnop',  year='$yr', type='FA', created=now() ");
