...
$ach= $con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count FROM `myrecord` where 
                 receipt_date BETWEEN '$frm' AND '$t2' AND year='$y' AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch='FA Lagos' 
                   OR branch='FA Abuja' OR branch='FA PH') ");
$yr =$ach->fetch_array();
...
$ach2=$con->query("SELECT count(distinct agent_code) AS count FROM myrecord where 
                 receipt_date BETWEEN '$frm' AND '$t2' AND year='$y' AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch='Ensure Victoria Island' OR branch='Ensure Ikeja' OR
                  branch='Ensure Broad Street' OR branch='Ensure Abuja' OR branch='FA PH')");
...
$rAmount = $yr['sum'];
 ...
 $proName=$con->query("SELECT distinct branch FROM branch_bud ");
    ...
    //VI
    $covBudget = $con->query("SELECT sum(nb), sum(rn), sum(polnb), sum(polrn) FROM monthly WHERE month BETWEEN '$d1' AND '$d2' AND bID='$bid' AND branch ='$bra' ");
    ...
    */
    ...
    $covArea = $con->query("SELECT sum(r_amount) AS amount, count(distinct policy_no) AS count, agent_code, agent_name FROM `myrecord` WHERE 
      receipt_date BETWEEN '$frm' AND '$t2' AND year='$yy' AND agent_code IN (SELECT agent_code FROM agent WHERE branch = '$bra') GROUP BY agent_name ");
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                   <div class="col-md-12 col-xs-12 col-sm-6">
                        <div class="white-box">
                            <h3 class="box-title">Budget vs Actual Between <?php echo $frm." & ". $t2; ?></h3>
...
<!--                                                        <th width="10%"><strong>RN Budget</strong></th>
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        <th scope="row">
                                                            <strong>
                                                                <a href="fa-w_agt.php?code=<?php echo $agCode ?>&bran=<?php echo $bra;?>&budg=<?php echo $bid ?>&frm=<?php echo $frm ?>&to=<?php echo $t2 ?>&year=<?php echo $yy ?>&r=<?php echo $rol ?>" ><strong> <?php echo $faName; ?></strong></a>
                                                            </strong>
                                                        </th>
                                    ...
                                    <table class="table table-bordered mb-0" width="974" height="133" border="1">
                                        ...
                                        while($cr = $cp->fetch_array()){
                                            ...
                                            while($ro=$cpp->fetch_array()){
                                                ...
                                                //Activization for PH
                                                $acvtAbj = $con->query("SELECT count(distinct agent_code) AS agent, count(distinct policy_no) AS count, product_class FROM `myrecord` WHERE receipt_date BETWEEN '$frm' AND '$t2' AND product_class='$prodt' AND type='RN' AND year='$yy' AND  r_amount >= 10000 AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch='$bra'  ) ");
                                                $ar=$acvtAbj->fetch_array();
                                                $act= $ar['agent'];
                                            }
