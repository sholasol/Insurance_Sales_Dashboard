...
$agcy=$con->query("SELECT sum(nb) AS nb, sum(rn) AS rn, sum(polnb) AS polnb, sum(polrn) AS polrn FROM proclass_bud WHERE bID='$bid' AND type='Financial Advisors' AND year='$y'");
$rw=$agcy->fetch_array();
$tBud=$rw['nb'] + $rw['rn'];
...
$ach= $con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count FROM `myrecord` where 
                  year='$y' AND c_area LIKE '%Financial Advisors%' GROUP BY product_class");
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                <div class="row bg-title">
                    ...
                    <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
                        <ol class="breadcrumb">
                            <li class="active"><a href="fa_budget.php" style="color: #005983;"><i class="icon-briefcase"></i> Create Budget</a> </li>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped">
                                    ...
                                    <tbody>
                                        ...
                                        $per=$con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count, c_area FROM `myrecord` where c_area LIKE '%Financial Advisors%' AND year='$y' 
                                                        AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch='Fa Lagos' OR branch='FA Abuja' OR branch='FA PH') GROUP BY c_area ORDER BY sum DESC");
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped">
                                    ...
                                    <tbody>
                                        ...
                                        $agtPer=$con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count, agent_name, agent_code FROM `myrecord` where c_area LIKE '%Financial Advisors%' AND year='$y' AND agent_name !='' GROUP BY agent_code ORDER BY sum DESC LIMIT 5 ");
           ...
           <?php
            ...
            while($pr=$p->fetch_array()){
               ...
               // $q1[] = 0;
            $ach= $con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count FROM `myrecord` where 
             product_class= '$produt' AND year='$y' AND type='NB' AND c_area LIKE '%Financial Advisors%' ");
            while($yr =$ach->fetch_array()){
            ...
            $xxx= $yr['sum']/1000000;
             ...
             product_class= '$produt' AND year='$y' AND type='RN' AND c_area LIKE '%Financial Advisors%' ");
