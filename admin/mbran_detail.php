...
$ach= $con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count FROM `myrecord` where 
                 month='$m' AND year='$y' AND type='NB' AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch='$bra') ");
$yr =$ach->fetch_array();
$polNB=$yr['count'];
$AmtNB = $yr['sum'];
...
$ach2=$con->query("SELECT count(distinct agent_code) AS agent, count(distinct policy_no) AS count, sum(r_amount) AS sum FROM myrecord where 
                 month='$m' AND year='$y' AND type='RN' AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch='$bra')");
                    ...
                    <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
                        <ol class="breadcrumb">
                            ...
                            if($_GET['b'] =='VI'){
                                ...
                                elseif($_GET['b']=='IB'){
                                    echo "<a href='m_ibadan.php?m=$m&r=$rol' class='card-title'> <i class='icon-home'></i> Back</a>";
                                }
                                elseif($_GET['b']=='FIB'){
                                    echo "<a href='m_ibadan.php?m=$m&r=$rol' class='card-title'> <i class='icon-home'></i> Back</a>";
                                }
                 ...
                 <!--  <div class="col-md-12 col-xs-12 col-sm-6">
                    ...
                    <div class="col-xs-12"> 
                        <div class="col-md-12" style="margin-bottom: 15px;"> 
                            <form action="bbbm.php" method="post"> 
                                <input type="hidden" name="role" value="<?php echo $rol ?>" />
                                <input type="hidden" name="branch" value="<?php echo $bra ?>" />
