...
$ach= $con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count, product_class FROM `myrecord` where 
             c_area LIKE '$area' AND year='$y' AND type='NB' AND agent_code IN (SELECT agent_code FROM agent WHERE branch=$branch) GROUP BY product_class");
...
year='$y'  AND agent_code IN (SELECT agent_code FROM agent WHERE branch=$branch) GROUP BY product_class");
...
$ach= $con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count FROM `myrecord` where 
 c_area LIKE '$area' AND   year='$y' AND type='NB' AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch=$branch) ");
$yr =$ach->fetch_array();
...
c_area LIKE '$area' AND year='$y' AND type='RN' AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch=$branch)");
...
$AmtRN = $yr2['sum'];
 ...
 //Coverage Areas Budget
    ...
    $covArea = $con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count FROM `myrecord` where 
     c_area LIKE '$area' AND year='$y' AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE  branch =$branch ) ");
    $cv = $covArea->fetch_array();
    $suAmt = $cv['sum'];
    $suPol = $cv['count'];
    
    //Number of RSEs in Each Branch
    $ag=$con->query("SELECT count(distinct agent_code) AS count FROM `myrecord` where c_area LIKE '$area' AND year='$y' AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch=$branch) ");
            ...
            <div class="container-fluid">
                <div class="row bg-title">
                    ...
                    <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
                        <ol class="breadcrumb">
                            <li class="active">
                                <?php 
                                 echo "<a href='pytd.php?r=$rol'><i class='icon-home'></i> Back</a> ";
                                ?>
                            </li>
                        </ol>
                    </div>
                ...
                <div class="row">
                   <div class="row">
                                 ...
                                 <h3 class="box-title">RSEs</h3>
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-striped" width="100%">
                                                ...
                                                <tbody>
                                                    <?php
                                                    $q=$con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count, agent_name, agent_code FROM `myrecord` where c_area LIKE '$area' AND year='$y' AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE  branch=$branch ) GROUP BY agent_name ORDER BY agent_name DESC");
                                                    while($row = $q->fetch_array()){
                                                        $code=$row['agent_code'];
                                                    ?>
                                                    <tr>
                                                        <th><strong><a href="agent.php?code=<?php echo $code ?>&r=<?php echo $rol ?>"><?php echo $row['agent_name']; ?></a></strong></th>
                    ...
                    <?php
                    //$con->query("SELECT count(distinct agent_code) AS count FROM `myrecord` where c_area LIKE '$area' AND year='$y' AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch=$branch) ");
                   /*
                    <div class="row">
                        <div class="col-md-12 col-xs-12 col-sm-6">
                             <div class="white-box">
