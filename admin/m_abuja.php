...
$rAmount = $yr['sum'];
 ...
 //Branch Wise Budget and Policy
    ...
    //Lagos
    $covBudget1 = $con->query("SELECT sum(nb), sum(rn), sum(polnb), sum(polrn) FROM branch_bud WHERE bID='$bud_id' AND branch LIKE '%Lagos%' OR branch LIKE '%Yaba%' OR branch LIKE '%Ikeja%' OR branch LIKE '%Apapa%' OR branch LIKE '%Ensure Broad Street%' OR branch LIKE '%Ensure Victoria Island%' ");
    ...
    //Abuja
    $covArea = $con->query("SELECT count(distinct policy_no) AS count, sum(r_amount) FROM myrecord WHERE month='$m' AND year='$yy' AND agent_code IN (SELECT agent_code FROM agent WHERE branch ='$brac1'  ) ");
    $cv = $covArea->fetch_array();
    $suAmt = $cv['sum(r_amount)'];
    $suPol = $cv['count'];
    
    //Lagos
    $covArea1 = $con->query("SELECT count(distinct policy_no) AS count, sum(r_amount) FROM myrecord WHERE month='$m' AND year='$yy' AND c_area LIKE '%Lagos%' OR c_area LIKE '%Ikeja%' OR c_area LIKE '%Yaba%'  OR c_area LIKE '%Onikan%' OR c_area LIKE '%Broad Street%' OR c_area LIKE '%Ikeja%'     ");
    $cv1 = $covArea1->fetch_array();
    $suAmt1 = $cv1['sum(r_amount)'];
    $suPol1 = $cv1['count'];
    
    
    //PH
    $covArea2 = $con->query("SELECT count(distinct policy_no) AS count, sum(r_amount) FROM myrecord WHERE month='$m' AND year='$yy' AND c_area LIKE '%Port Harcourt%' ");
            ...
            <div class="container-fluid">
                <div class="row bg-title">
                    ...
                    <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
                        <ol class="breadcrumb">
                            <li class="active"><a href="month.php?m=<?php echo $m; ?>&r=<?php echo $rol; ?>"><i class="icon-home"></i> Back</a> </li>
                        </ol>
                    </div>
                    <!-- /.col-lg-12 -->
                </div>
                ...
                <div class="row">
                   <div class="col-md-12 col-xs-12 col-sm-6">
                        <div class="white-box">
                            <h3 class="box-title">Abuja</h3>
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered" width="100%">
                                                <thead>
                                                    <tr>
                                                        <th width="25%">Coverage Area</strong></th>
                                                        <th width="10%">Bgdt</strong></th>
                                                ...
                                                <tbody>
                                                    <tr>
                                                        <th>
                                                            <strong>
                                                                <?php 
                                                                    if($rol =='Agency'){echo"<a href='mbran_detail.php?m=$m&b=ABJ&r=$rol'>Abuja (Supervisor Wise)</a> ";}
                                                                    if($rol =='FA'){echo"<a href='mbran_detail.php?m=$m&b=FABJ&r=$rol'>FA Abuja </a> ";}
                                                                ?>
                                                            </strong>
                                                        </th>
                                                        <th><strong><span class=""><?php echo number_format($sumBud); ?></span></strong></th>
                                                        <th><strong><span class=""><?php echo number_format($suAmt); ?></span></strong></th>
                                                        <th><strong><span class=""> <?php echo number_format($PA_Abuja, 2, '.', '')."%"; ?></span> </strong></th>
                                                        <th><strong><span class=""><?php echo number_format($sumPol); ?></span></strong></th>
                                                        <th><strong><span class=""><?php echo number_format($suPol); ?></span></strong></th>
                                                        <th><strong><span class=""> <?php echo number_format($PA_Abuja1, 2, '.', '')."%"; ?></span></strong></th>
                                                    </tr>
                    ...
                    <div class="col-xs-12"> 
                        <div class="col-md-12" style="margin-bottom: 15px;"> 
                            <form action="bbm.php" method="post"> 
                                ...
                                <input type="hidden" name="branch" value="Abuja" />
                                <button class="btn btn-block btn-info" type="submit" name="bb"><i class="icon-doc"></i> Export to Excel</button>
                            </form> 
                        </div>
                        <div class="col-xs-5 ">
                            <div class="white-box">
                                <h3 class="box-title"> Budget (Abuja)</h3>
                                <div class="table-responsive">
                                <table class="table table-bordered mb-0">
                                              ...
                                              <?php 
                                                ...
                                                while($rr2=$p2->fetch_array()){
                                                    ...
                                                    $pp2=$con->query("SELECT nb, rn, polnb, polrn FROM monthly WHERE product_class='$pr2' AND bID='$bid' AND month='$m' AND branch='$brac1'");
                                                    $ro2=$pp2->fetch_array();
                                                    $nb2=$ro2['nb'];
                                                    $newBizBud +=$nb2;
                                                    $rn2=$ro2['rn'];
                        ...
                        <div class="col-xs-7 " >
                            <div class="white-box">
                                <h3 class="box-title">Budget vs Actual (Abuja)</h3>
                                <div class="table-responsive">
...
<!--                                          <th colspan="4" align="center" bgcolor="#66FF99">Ibadan</strong></th>-->
                                        ...
                                        while($cr = $cp->fetch_array()){
                                            ...
                                            $pb = $con->query("SELECT nb, rn, polnb, polrn, product_class FROM `branch_bud` WHERE product_class='$prodt' AND branch ='$brac1' AND year='$yy'");
                                            $or = $pb->fetch_array();
                                            $nbB = $or['nb'];
                                            $rnB = $or['rn'];
                                            $nbP = $or['polnb'];
                                            ...
                                            while($ro=$cpp->fetch_array()){
                                                ...
                                                //Activization for PH
                                                $acvtAbj = $con->query("SELECT count(distinct agent_code) AS agent, count(distinct policy_no) AS count, product_class FROM `myrecord` WHERE product_class='$prodt' AND type='NB' AND month='$m' AND year='$yy' AND  r_amount >= 10000 AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch='$brac1'  ) ");
                                            ...
                                            $cpp1 = $con->query("SELECT sum(premium) AS premium, sum(r_amount) AS amount,count(distinct agent_code) AS agent, count(distinct policy_no) AS count, product_class FROM `myrecord` WHERE product_class='$prodt' AND type='RN' AND month='$m' AND year='$yy' 
                                            AND agent_code IN (SELECT agent_code FROM agent WHERE branch ='$brac1'  ) ");
                                            while($ro1=$cpp1->fetch_array()){
                                                ...
                                                //Activization for PH
                                                $acvtAbj1 = $con->query("SELECT count(distinct agent_code) AS agent, count(distinct policy_no) AS count, product_class FROM `myrecord` WHERE product_class='$prodt' AND type='RN' AND month='$m' AND year='$yy'  AND r_amount >= 10000 AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch='$brac1'  ) ");
