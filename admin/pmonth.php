...
if(isset($_POST['submit'])) {
    if(empty($_POST['month'])){
        ...
        $mn=$_POST['month'];
        echo  " <script> window.location='pmonth.php?m=$mn&r=$rol' </script>";
...
$agcy=$con->query("SELECT sum(nb) AS nb, sum(rn) AS rn, sum(polnb) AS polnb, sum(polrn) AS polrn FROM proclass_bud WHERE bID='$bid' AND  year='$y'");
$rw=$agcy->fetch_array();
$tBud=$rw['nb'] + $rw['rn'];
$tPol=$rw['polnb'] + $rw['polrn'];

$ach= $con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count FROM `myrecord` where 
                 month='$m' AND c_area ='$area' AND year='$y' AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch='Ensure Victoria Island' OR branch='Ensure Ikeja' OR
...
$rAmount = $yr['sum'];
 ...
 $proName=$con->query("SELECT distinct branch FROM branch_bud ");
    ...
    //Abuja
    $covBudget = $con->query("SELECT sum(nb), sum(rn), sum(polnb), sum(polrn) FROM monthly WHERE bID='$bud_id' AND month='$m'  ");
    $cor=$covBudget->fetch_array();
    $sumBud =$cor['sum(nb)'] + $cor['sum(rn)'];
    $sumPol =$cor['sum(polnb)'] + $cor['sum(polrn)'];
    
    //Lagos
    $covBudget1 = $con->query("SELECT sum(nb), sum(rn), sum(polnb), sum(polrn) FROM monthly WHERE bID='$bud_id' AND month='$m' ");
    ...
    //PH
    $covBudget2 = $con->query("SELECT sum(nb), sum(rn), sum(polnb), sum(polrn) FROM monthly WHERE bID='$bud_id' AND month='$m'  ");
    ...
    $covArea = $con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count FROM `myrecord` where 
                month='$m' AND c_area ='$area' AND  year='$yy' AND `agent_code` IN (SELECT agent_code FROM `agent`  ) ");
    ...
    $covArea1 = $con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count FROM `myrecord` where 
              month='$m' AND c_area ='$area' AND   year='$yy' AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch = $branch) ");
                        ...
                        <div class="white-box">
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered" width="100%">
                                                ...
                                                <tbody>
                                                    <tr>
                                                        <th><strong><a href="mdetail.php?m=<?php echo $m; ?>&r=<?php echo $rol; ?>" class="btn-link" style="font-size: 17px;">Sales Performance</a> </strong></th>
                                                        ...
                                                        if($PA_Abuja < 1){echo number_format($PA_Abuja, 2, '.', '')."%"; }else{ echo number_format($PA_Abuja, 2, '.', '')."%";  }?>
                                                         </span></strong> </strong>
                                                        </th>
                                                        ...
                                                        <th><strong><strong><span class=""> <?php 
                                                        if($PA_Abuja1 < 1){echo number_format($PA_Abuja1, 2, '.', '')."%"; }else{ echo number_format($PA_Abuja1, 2, '.', '')."%";  }?>
                                                         </span></strong> </strong>
                                                        </th>
                                                    </tr>
                            ...
                            <div class="white-box">
                                ...
                                <tbody>
                                    ...
                                    $p_bgt=$con->query("SELECT  product_class FROM `myrecord` WHERE  year='$yy'  GROUP BY product_class");
                                     ...
                                     $bugt_P = $con->query("SELECT sum(nb) AS nb, sum(polnb) AS polnb FROM monthly WHERE product_class='$prdt' AND month='$m' AND year='$yy'  ");
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered mb-0" width="974" height="133" border="1">
                                        ...
                                        while($cr = $cp->fetch_array()){
                                            ...
                                            $cpp1 = $con->query("SELECT sum(premium) AS premium, sum(r_amount) AS amount, count(distinct agent_code) AS agent, count(distinct policy_no) AS count, product_class FROM `myrecord` WHERE month='$m' AND product_class='$prodt' AND type='NB' AND c_area ='$area' AND  year='$yy' 
                                            ...
                                            $acvtLag = $con->query("SELECT count(distinct agent_code) AS agent, count(distinct policy_no) AS count, product_class FROM `myrecord` WHERE month='$m' AND  product_class='$prodt' AND type='NB' AND c_area ='$area' AND year='$yy' AND r_amount >= 10000 AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch=$branch  ) ");
                            ...
                            <div class="white-box">
                                ...
                                <tbody>
                                    ...
                                    $p_bgt=$con->query("SELECT  product_class FROM `myrecord` WHERE  year='$yy'  GROUP BY product_class");
                                     ...
                                     $bugt_P = $con->query("SELECT sum(rn) AS rn, sum(polrn) AS polrn FROM monthly WHERE product_class='$prdt' AND month='$m' AND year='$yy' ");
                                            $rr = $bugt_P->fetch_array();
...
<!--                                          <td bgcolor="#66FF99"><h3 align="center" style="color: #fff; font-size: 17px;">Ach</h3></strong></th>
                                        ...
                                        while($cr = $cp->fetch_array()){
                                            ...
                                            $cpp1 = $con->query("SELECT sum(premium) AS premium, sum(r_amount) AS amount, count(distinct agent_code) AS agent, count(distinct policy_no) AS count, product_class FROM `myrecord` WHERE month='$m' AND  product_class='$prodt' AND type='RN' AND c_area ='$area' AND  year='$yy' 
                                            AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch= $branch ) ");
                                            $ro1=$cpp1->fetch_array();
                                            $proAmt1 = $ro1['amount'];
                                            $proPol1 = $ro1['count'];
                                            ...
                                            $nopLag += $proPol1;
                                            // CAlculating Activization
                                            $acvtLag = $con->query("SELECT count(distinct agent_code) AS agent, count(distinct policy_no) AS count, product_class FROM `myrecord` WHERE month='$m' AND  product_class='$prodt' AND type='RN' AND c_area ='$area' AND year='$yy' AND r_amount >= 10000 AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch= $branch  ) ");
                                            $ar1=$acvtLag->fetch_array();
                                            $act1= $ar1['agent'];
