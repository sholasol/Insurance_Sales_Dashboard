...
$rol = $_GET['r'];
    ...
    //VI
    $covBudgetL = $con->query("SELECT sum(nb), sum(rn), sum(polnb), sum(polrn) FROM monthly WHERE bID='$bud_id' AND month='$m' AND branch ='Ensure Victoria Island' ");
    $corL=$covBudgetL->fetch_array();
    $sumBudL =$corL['sum(nb)'] + $corL['sum(rn)'];
    $sumPolL =$corL['sum(polnb)'] + $corL['sum(polrn)'];
    
    //Ikeja
    $covBudget1L = $con->query("SELECT sum(nb), sum(rn), sum(polnb), sum(polrn) FROM monthly WHERE bID='$bud_id' AND month='$m' AND branch ='Ensure Ikeja' ");
    ...
    //B/Street
    $covBudget2L = $con->query("SELECT sum(nb), sum(rn), sum(polnb), sum(polrn) FROM monthly WHERE bID='$bud_id' AND month='$m' AND branch='Ensure Broad Street'  ");
    ...
    month='$m' AND year='$yy' AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE  branch='Ensure Victoria Island' ) ");
    ...
    month='$m' AND year='$yy' AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch='Ensure Ikeja') ");
    ...
    month='$m' AND year='$yy' AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch='Ensure Broad Street') ");
                                 ...
                                 <h3 class="box-title">Branch Wise (Branch)</h3>
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-striped" width="100%">
                                                   ...
                                                   <tbody>
                                                       <tr>
                                                           <th><strong><a href="mbran_detail.php?b=VI&m=<?php echo $m; ?>&r=<?php echo $rol; ?>">Victoria Island (Supervisor Wise)</a></strong> </th>
                                                       ...
                                                       <tr>
                                                           <th><strong><a href="mbran_detail.php?b=IKJ&m=<?php echo $m; ?>&r=<?php echo $rol; ?>">Ikeja (Supervisor Wise) </a></strong></th>
                                                       ...
                                                       <tr>
                                                           <th><strong><a href="mbran_detail.php?b=BS&m=<?php echo $m; ?>&r=<?php echo $rol; ?>">Broad Street (Supervisor Wise) </a></strong></th>
