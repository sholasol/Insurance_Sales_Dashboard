 ...
 $proName=$con->query("SELECT distinct branch FROM branch_bud ");
   ...
   //Coverage Areas Budget
    ...
    $covArea = $con->query("SELECT sum(r_amount) as sum, count(distinct policy_no) AS count FROM `myrecord` where 
                receipt_date BETWEEN '$frm' AND '$t2' AND  year='$yy' AND c_area ='$area' AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE  branch='$brac1' ) ");
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered mb-0" width="974" height="133" border="1">
                                        ...
                                        while($cr = $cp->fetch_array()){
                                            ...
                                            AND c_area ='$area' AND `agent_code` IN (SELECT agent_code FROM `agent` WHERE branch='$brac1' ) ");
                                            $ro=$cpp->fetch_array();
                                            $proAmt = $ro['amount'];
                                            $proPol = $ro['count'];
                                            $agent = $ro['agent'];
